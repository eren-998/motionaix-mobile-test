...
export type AsciiRemotionProps = {
  imageUrl?: string;
  charSize?: number;
  colorMode?: "green" | "amber" | "original" | "bw";
  splitPos?: number;
  showSplit?: boolean;
  transparentBg?: boolean;
};

export const AsciiRemotion: React.FC<AsciiRemotionProps> = ({
  ...
  useEffect(() => {
    ...
    const size = Math.max(6, charSize);
    const chars = ["@", "#", "$", "%", "*", "!", ";", ":", ",", "."];
    ...
    for (let y = 0; y < h; y += size) {
      for (let x = 0; x < w; x += size) {
        ...
        if (colorMode === "green") ctx.fillStyle = "#22c55e";
        else if (colorMode === "amber") ctx.fillStyle = "#f59e0b";
        else if (colorMode === "original") ctx.fillStyle = `rgb(${r},${g},${b})`;
  ...
  }, [imageLoaded, frame, charSize, colorMode, showSplit, splitPos, transparentBg, compWidth, compHeight]);
