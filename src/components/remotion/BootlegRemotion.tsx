...
export type BootlegRemotionProps = {
  imageUrl?: string;
  pixelSize?: number;
  colorShift?: boolean;
  edgeOutline?: boolean;
...
export const BootlegRemotion: React.FC<BootlegRemotionProps> = ({
  ...
  useEffect(() => {
    ...
    for (let y = 0; y < h; y += size) {
      for (let x = 0; x < w; x += size) {
        ...
        if (lum < 0.85) {
          let r = data[idx] > 100 ? 255 : 180;
          let g = data[idx + 1] > 120 ? 200 : 20;
          let b = 10;

          if (colorShift && lum < 0.35) {
            r = 230;
            g = 30;
            b = 30;
          }
  ...
  }, [imageLoaded, frame, pixelSize, colorShift, edgeOutline, showSplit, splitPos, transparentBg, compWidth, compHeight]);
